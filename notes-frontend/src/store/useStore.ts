...
// 1. Single source of truth for your API base URL
const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000/api';
...
// Define what a queued action looks like
interface SyncAction {
  id: string;
  url: string;
  method: 'POST' | 'PUT' | 'DELETE';
  ...
  persist(
    (set, get) => ({
      ...
      fetchFolders: async (userId) => {
        if (!get().isOnline) return;
        try {
          const res = await fetch(`${API_URL}/folders?userId=${userId}`);
          if (res.ok) {
            const cloudFolders = await res.json();
            set({ folders: [{ id: 'default', name: 'Notes', createdAt: Date.now() }, ...cloudFolders] });
          }
      ...
      addFolder: (name, userId) => {
        ...
        if (get().isOnline) {
          fetch(`${API_URL}/folders`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        } else {
          set(state => ({ actionQueue: [...state.actionQueue, { id: crypto.randomUUID(), url: `${API_URL}/folders`, method: 'POST', body: payload }] }));
        }
      },
      ...
      addNote: (userId) => {
        ...
        // 2. Network Check
        if (get().isOnline) {
          fetch(`${API_URL}/notes`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        } else {
          set(state => ({ actionQueue: [...state.actionQueue, { id: crypto.randomUUID(), url: `${API_URL}/notes`, method: 'POST', body: payload }] }));
        }
      },
